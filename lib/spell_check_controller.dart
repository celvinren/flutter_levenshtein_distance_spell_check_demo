...
import 'package:flutter_levenshtein_distance_spell_check_demo/functions.dart';
...
class SpellCheckController extends TextEditingController {
  ...
  final isolateFindMistakes = IsolateManager.create(
    findMistakes,
    // concurrent: 4,
  );
  ...
  set value(TextEditingValue newValue) {
    ...
    timer = Timer(const Duration(milliseconds: 600), () {
      isolateFindMistakes
          .sendMessage([locale.languageCode, text, checkedWords]);
    });
  }
  ...
  void dispose() {
    timer?.cancel();
    isolateFindMistakes.stop();
    super.dispose();
  }

  Future<void> initial() async {
    await isolateFindMistakes.start();
    isolateFindMistakes.stream.listen((result) {
      final foundMistakes = result.$1;
      final modifiedCheckedWords = result.$2;
      checkedWords = Set<String>.from(modifiedCheckedWords);
      mistakes = [
        ...mistakes,
        ...foundMistakes
  ...
  // void _findMistakes() {
  //   isolateFindMistakes.sendMessage([locale.languageCode, text, checkedWords]);
  // }
